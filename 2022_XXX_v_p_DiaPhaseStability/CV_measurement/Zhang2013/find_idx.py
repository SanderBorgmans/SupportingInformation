...
def get_idx(sys, do_return=True):
    ...
    for k,v in sets.items():
        ...
        for i in v:
            ...
            if np.linalg.norm(vec - np.sign(vec[0])*np.sign(rvecs[0][0])*rvecs[0]) < np.linalg.norm(vec):
                #print('old: ',sys.pos[i],sys.pos[c4_i],vec)
                sys.pos[i] -= np.sign(vec[0])*np.sign(rvecs[0][0])*rvecs[0]
                vec -= np.sign(vec[0])*np.sign(rvecs[0][0])*rvecs[0]
                #print('new_x: ',sys.pos[i],sys.pos[c4_i],vec)
            if np.linalg.norm(vec - np.sign(vec[1])*np.sign(rvecs[1][1])*rvecs[1]) < np.linalg.norm(vec):
                #print('old: ',sys.pos[i],sys.pos[c4_i],vec)
                sys.pos[i] -= np.sign(vec[1])*np.sign(rvecs[1][1])*rvecs[1]
                vec -= np.sign(vec[1])*np.sign(rvecs[1][1])*rvecs[1]
                #print('new_y: ',sys.pos[i],sys.pos[c4_i],vec)
            vecs.append(vec)
