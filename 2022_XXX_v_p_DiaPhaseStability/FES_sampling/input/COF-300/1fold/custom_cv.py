...
def get_cv(ff):
    cv_ohb = CVDistanceProjection(ff.system, [np.array([218]),np.array([289])]) # outer  horizontal bottom
    cv_oht = CVDistanceProjection(ff.system, [np.array([142]),np.array([69])]) # outer horizontal top

    cv_ivl = CVDistanceProjection(ff.system, [np.array([143]),np.array([216])]) # inner vertical left
    cv_ivr = CVDistanceProjection(ff.system, [np.array([71]),np.array([288])]) # inner vertical right

    # Cat 2: compresses when rotating
    cv_ihb = CVDistanceProjection(ff.system, [np.array([217]),np.array([290])])
    cv_iht = CVDistanceProjection(ff.system, [np.array([145]),np.array([70])])

    cv_ovl = CVDistanceProjection(ff.system, [np.array([144]),np.array([215])])
    cv_ovr = CVDistanceProjection(ff.system, [np.array([72]),np.array([291])])
