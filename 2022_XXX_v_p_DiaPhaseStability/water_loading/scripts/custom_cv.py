...
def get_cv(ff):
    cv_ohb = CVDistanceProjection(ff.system, [np.array([187]),np.array([290])])
    cv_oht = CVDistanceProjection(ff.system, [np.array([42]),np.array([143])])
    cv_ivl = CVDistanceProjection(ff.system, [np.array([188]),np.array([44])])
    cv_ivr = CVDistanceProjection(ff.system, [np.array([288]),np.array([142])])
    cv_ihb = CVDistanceProjection(ff.system, [np.array([190]),np.array([291])])
    cv_iht = CVDistanceProjection(ff.system, [np.array([41]),np.array([144])])
    cv_ovl = CVDistanceProjection(ff.system, [np.array([189]),np.array([43])])
    cv_ovr = CVDistanceProjection(ff.system, [np.array([289]),np.array([145])])
...
def get_pore_com(sys):
    c4_id = np.where(sys.ffatypes == 'C_C4_C8_TAM')[0]
    c4_idx = np.where(sys.ffatype_ids == c4_id)[0]
    return np.average(sys.pos[c4_idx],axis=0)
