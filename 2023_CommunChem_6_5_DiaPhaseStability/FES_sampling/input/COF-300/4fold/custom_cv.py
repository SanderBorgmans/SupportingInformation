...
def get_cv(ff):
    # Select CV(s)
    cv_ohb = CVDistanceProjection(ff.system, [np.array([187]),np.array([290])]) # outer  horizontal bottom
    cv_oht = CVDistanceProjection(ff.system, [np.array([42]),np.array([143])]) # outer horizontal top

    cv_ivl = CVDistanceProjection(ff.system, [np.array([44]),np.array([188])]) # inner vertical left
    cv_ivr = CVDistanceProjection(ff.system, [np.array([142]),np.array([288])]) # inner vertical right
    ...
    cv_ovl = CVDistanceProjection(ff.system, [np.array([43]),np.array([189])])
    cv_ovr = CVDistanceProjection(ff.system, [np.array([145]),np.array([289])])
