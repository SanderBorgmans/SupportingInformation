...
def get_cv(ff):
    ...
    cv_ohb = CVDistanceProjection(ff.system, [np.array([44]),np.array([188])]) # outer  horizontal bottom
    cv_oht = CVDistanceProjection(ff.system, [np.array([142]),np.array([290])]) # outer horizontal top

    cv_ivl = CVDistanceProjection(ff.system, [np.array([144]),np.array([41])]) # inner vertical left
    cv_ivr = CVDistanceProjection(ff.system, [np.array([289]),np.array([190])]) # inner vertical right

    # Cat 2: compresses when rotating
    cv_ihb = CVDistanceProjection(ff.system, [np.array([43]),np.array([189])])
    cv_iht = CVDistanceProjection(ff.system, [np.array([145]),np.array([291])])

    cv_ovl = CVDistanceProjection(ff.system, [np.array([143]),np.array([42])])
    cv_ovr = CVDistanceProjection(ff.system, [np.array([288]),np.array([187])])
