...
def get_cv(ff):
    cv_ohb = CVDistanceProjection(ff.system, [np.array([190]),np.array([288])]) # outer  horizontal bottom
    cv_oht = CVDistanceProjection(ff.system, [np.array([143]),np.array([41])]) # outer horizontal top

    cv_ivl = CVDistanceProjection(ff.system, [np.array([42]),np.array([189])]) # inner vertical left
    cv_ivr = CVDistanceProjection(ff.system, [np.array([142]),np.array([289])]) # inner vertical right

    # Cat 2: compresses when rotating
    cv_ihb = CVDistanceProjection(ff.system, [np.array([188]),np.array([290])])
    cv_iht = CVDistanceProjection(ff.system, [np.array([43]),np.array([145])])

    cv_ovl = CVDistanceProjection(ff.system, [np.array([44]),np.array([187])])
    cv_ovr = CVDistanceProjection(ff.system, [np.array([144]),np.array([291])])
