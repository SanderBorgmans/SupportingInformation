...
def get_cv(ff):
    ...
    cv_ivl = CVDistanceProjection(ff.system, [np.array([188]),np.array([44])]) # inner vertical left
    cv_ivr = CVDistanceProjection(ff.system, [np.array([290]),np.array([142])]) # inner vertical right
    ...
    cv_ovr = CVDistanceProjection(ff.system, [np.array([291]),np.array([145])])
