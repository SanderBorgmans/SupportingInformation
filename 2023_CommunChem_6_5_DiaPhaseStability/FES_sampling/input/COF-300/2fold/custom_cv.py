...
def get_cv(ff):
    cv_ohb = CVDistanceProjection(ff.system, [np.array([187]),np.array([288])]) # outer  horizontal bottom
    ...
    cv_ivr = CVDistanceProjection(ff.system, [np.array([142]),np.array([290])]) # inner vertical right

    # Cat 2: compresses when rotating
    cv_ihb = CVDistanceProjection(ff.system, [np.array([190]),np.array([289])])
