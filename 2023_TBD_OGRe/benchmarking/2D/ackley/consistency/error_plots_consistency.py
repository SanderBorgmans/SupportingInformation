...
if not os.path.exists('error_data/grid_data.pkl'):
    ...
    scan_dirs = glob.glob('scans_*_*_*_*_*')
    ...
    for index,sd in enumerate(scan_dirs):
        ...
        spacing,kappa,kg,consistency,js_thr = combinations[index]
        grids[(spacing,kappa,kg,consistency,js_thr)] = grid
        grids_error[(spacing,kappa,kg,consistency,js_thr)] = grid_error
        grids_info[(spacing,kappa,kg,consistency,js_thr)] = [CONFINEMENT, OVERLAP, MAX_LAYERS]
...
max_sims = np.max([np.nanmax(g[:,:,:,1]) for g in grids.values()])
min_sims = np.min([np.nanmin(g[:,:,:,1]) for g in grids.values()])
    ...
    for n,mi in enumerate(MAX_LAYERS):
        ...
        for i in range(grid.shape[0]):
            for j in range(grid.shape[1]):
                ...
                if grid[i,j,n,3]>1440: # more than 10% nans
                    axgrid[n].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="...", fill=False, snap=False))
...
def make_error_simulations_figure_extension(key,grid,grid_error,error_index=0):
    num_plots_per_row = 5
    nrows = int(np.ceil(float(grid.shape[-2])/num_plots_per_row)) * 3 # triple the number for the simulations, and simulation time
    ncols = num_plots_per_row
    ...
    for n,mi in enumerate(MAX_LAYERS):
        im0 = axgrid[indices2index(0,n)].imshow(grid_error[:,:,n,error_index].T, origin='lower', cmap='RdYlGn_r', vmin=0., vmax=5.) # norm=colors.LogNorm(vmin=min_error, vmax=max_error)
        im1 = axgrid[indices2index(1,n)].imshow(grid[:,:,n,1].T, origin='lower', cmap='RdYlGn_r', norm=colors.LogNorm(vmin=10, vmax=10000))
        im2 = axgrid[indices2index(2,n)].imshow(grid[:,:,n,1].T * 0.1 , origin='lower', cmap='RdYlGn_r', norm=colors.LogNorm(vmin=1, vmax=1000)) # in nanosecond
        ...
        for i in range(grid.shape[0]):
            for j in range(grid.shape[1]):
                if grid[i,j,n,0]:
                    axgrid[indices2index(0,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch='///', fill=False, snap=False))
                    axgrid[indices2index(1,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch='///', fill=False, snap=False))
                    axgrid[indices2index(2,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch='///', fill=False, snap=False))
                if grid[i,j,n,2]:
                    axgrid[indices2index(0,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="xxx", fill=False, snap=False))
                    axgrid[indices2index(1,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="xxx", fill=False, snap=False))
                    axgrid[indices2index(2,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="xxx", fill=False, snap=False))
                if grid[i,j,n,3]>1440: # more than 10% nans
                    axgrid[indices2index(0,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="...", fill=False, snap=False))
                    axgrid[indices2index(1,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="...", fill=False, snap=False))
                    axgrid[indices2index(2,n)].add_patch(mpl.patches.Rectangle((i-.5, j-.5), 1, 1, hatch="...", fill=False, snap=False))
        ...
        axgrid[indices2index(1,n)].set_xticks(np.arange(grid.shape[0]))
        axgrid[indices2index(1,n)].set_yticks(np.arange(grid.shape[1]))
        axgrid[indices2index(2,n)].set_xticks(np.arange(grid.shape[0]))
        axgrid[indices2index(2,n)].set_yticks(np.arange(grid.shape[1]))

        axgrid[n].title.set_text('MAX_LAYERS = {}'.format(mi))
        axgrid[indices2index(2,n)].set_xticklabels(CONFINEMENT,rotation = 90)
        axgrid[indices2index(2,n)].set_xlabel('CONFINEMENT_THR')
    for i in range(nrows):
        axgrid[indices2index(i,0)].set_yticklabels(OVERLAP)
        axgrid[indices2index(i,0)].set_ylabel('OVERLAP_THR')
    ...
    if im2 is not None:
        cbar = axgrid.cbar_axes[2].colorbar(im2)#, ticks=[min_error, max_error])
        #cbar.ax.set_yticklabels(['lowest error', 'highest error']) 
        cbar.set_label("Total simulation time (ns)")

    pt.tight_layout()
    pt.savefig('error_plots/extended_sims_error_{}.pdf'.format("_".join(key)),bbox_inches='tight')
    pt.close()
...
def plot_error_vs_simulations(grids,grids_error,grids_info,error_index=0):
    ...
    for key,grid in grids.items():
        ...
        idx = ((grid[:,:,:,0]==0)&(grid[:,:,:,2]==0)&(grid[:,:,:,3]<=1440))
        #idx = grid[:,:,:,3]<=1440
...
def plot_densities(grids,grids_error,grids_info):
    ...
    # single plot over all parameter combinations, for a fixed initial spacing, initial kappa, and kappa growth factor
    num_cols = 6
    fig = pt.figure(figsize=(num_cols*3,1*3))
    axgrid = AxesGrid(fig, 111,
    ...
    vmins = [2.0,1.0,2,0,0,0]
    vmaxs = [2.0,1.0,2,1,1,1]

    for n in range(6): # spacing, kappa, kgf, confinement, overlap, consistency
        # Define grid
...
def consistency_variations(grids,grids_error):
    ...
    for key,comb in reduced_key_combinations.items():
        ...
        grid_ref = grids[tuple([*key,comb[0],'1.00'])][:,:,-1,:]
        grid_error_ref = grids_error[tuple([*key,comb[0],'1.00'])][:,:,-1,:]

        ref_idx = ((grid_ref[:,:,0]==0)&(grid_ref[:,:,2]==0)&(grid_ref[:,:,3]<=1440))

        for n in range(1,len(comb)):
            grid = grids[tuple([*key,comb[n],'1.00'])][:,:,-1,:]
            grid_error = grids_error[tuple([*key,comb[n],'1.00'])][:,:,-1,:]

            idx = ((grid[:,:,0]==0)&(grid[:,:,2]==0)&(grid[:,:,3]<=1440) & ref_idx)
        ...
        data[tuple([*key,'1.00'])] = [avg_rmse_list,avg_num_sims_list]
...
#Combined error and num simulations plots, with num simulations in log scale (specific number is not important, order of magnitude is)
for key,grid in grids.items():
    make_error_simulations_figure_extension(key,grid,grids_error[key],error_index=0)
