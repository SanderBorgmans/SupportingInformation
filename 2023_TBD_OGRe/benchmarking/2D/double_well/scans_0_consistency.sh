#!/bin/bash
#PBS -N ogre_scan_simple_potential_0
...
for i in  "0.25 0.1 2"; do
    ...
    scan_dir=scans_${spacing}_${k}_${kg}_0.96
            ...
            for i in 0.00 0.10 0.25 0.33 0.50 0.66 0.75 0.90 1.00; do
                for j in 0.00 0.10 0.25 0.33 0.50 0.66 0.75 0.90 1.00; do
                    ...
                    if [ ! -d "$directory" ]; then
                        ...
                        # initialize
                        cd ${directory}
                        python database_init_database.py $MAX_LAYERS $CONFINEMENT_THR $OVERLAP_THR $INIT_SPACING $INIT_KAPPA $KAPPA_GROWTH_FACTOR 
                        
                        
                        # while the run file contains more entries than the header
                        while [[ $(find run.txt -type f -size +25c 2>/dev/null) ]]; do
                            python database_simulate_load_log_database.py $INIT_SPACING $INIT_KAPPA $KAPPA_GROWTH_FACTOR
                            ...
                            if [ ! -f "grid_restart.txt" ]; then
                                ...
                                while [ -f "grid_restart.txt" ]; do
                                    ...
                                    python adapt_cores_database.py $INIT_SPACING $INIT_KAPPA $KAPPA_GROWTH_FACTOR
                                    wsub -batch run_database.pbs -data grid_restart.txt -epilog epilog.sh --master
